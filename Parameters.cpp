...
void Parameters::update(std::vector<bool> whichNonLin, int numLin) {
    ...
    for (int i = 0; i<12; i++){
        _preTrans[i].at(0) = (0.5+0.5*tmp.at(5*i))*cosf(tmp.at(5*i+2));
        _preTrans[i].at(1) = -(0.5+0.5*tmp.at(5*i+1))*sinf(tmp.at(5*i+2));
        _preTrans[i].at(2) = tmp.at(5*i+3);
        _preTrans[i].at(3) = (0.5+0.5*tmp.at(5*i))*sinf(tmp.at(5*i+1));
        _preTrans[i].at(4) = (0.5+0.5*tmp.at(5*i+1))*cosf(tmp.at(5*i+2));
